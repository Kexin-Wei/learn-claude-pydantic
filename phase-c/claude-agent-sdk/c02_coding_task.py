#!/usr/bin/env python3
"""Coding task — Build expense tracker CLI using Claude Agent SDK.

Run: uv run python phase-c/claude-agent-sdk/coding_task.py

Results saved to phase-c/claude-agent-sdk/results/
"""
import asyncio
import os
import shutil
...
async def run(workdir: str) -> dict:
    ...
    async for msg in query(
        ...
        if isinstance(msg, AssistantMessage):
            ...
            task_success = msg.subtype == "success" and len(out["expected_found"]) == len(EXPECTED_FILES)
            return {
                "SDK": "Claude Agent SDK",
                "Completed": "success" if task_success else "fail",
                "Turns": msg.num_turns,
...
def main() -> None:
    ...
    print("=" * 60)
    print("Coding Task: Claude Agent SDK")
