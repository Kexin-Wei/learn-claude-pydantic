...
async def probe_custom_tools() -> None:
    ...
    print("  → @tool + create_sdk_mcp_server() = SDK-native way to add in-process tools")
...
async def probe_structured_output() -> None:
    """Part B: Get structured JSON output."""
    print("\n" + "=" * 60)
...
async def probe_permission_control() -> None:
    ...
    print("  → disallowed_tools blocks specific tools; can_use_tool gives dynamic control")
...
async def main() -> None:
    ...
    await probe_permission_control()

    print("\n" + "=" * 60)
    print("KEY FINDINGS")
    print("=" * 60)
    print("  SDK-native primitives (not inherited from Claude Code):")
    print("    • @tool + create_sdk_mcp_server: in-process custom tools")
    print("    • output_format: structured JSON output with schema enforcement")
    print("    • can_use_tool: programmatic permission callback (needs ClaudeSDKClient)")
    print("    • allowed_tools / disallowed_tools: static tool filtering")
    print("    • hooks: lifecycle event callbacks (PreToolUse, PostToolUse, Stop, etc.)")
    print("    • session management: list, get, rename, tag sessions")
    print("    • enable_file_checkpointing + rewind_files: file change tracking")
    print("    • streaming: partial messages, StreamEvent, RateLimitEvent")
