...
def main() -> None:
    ...
    tools_a = extract_tools(messages)
    agent_used_a = any(t in tools_a for t in ("Agent", "Task"))
    ...
    print(f"\n  Tools: {dict(tools_a)}")
    print(f"  Agent/Task tool used: {'YES' if agent_used_a else 'NO'}")
    ...
    tools_b = extract_tools(messages)
    agent_used_b = any(t in tools_b for t in ("Agent", "Task"))
    ...
    print(f"\n  Tools: {dict(tools_b)}")
    print(f"  Agent tool used: {'YES' if agent_used_b else 'NO'}")

    print(f"\n{'=' * 60}")
    print("KEY FINDINGS")
    print("=" * 60)
    print(f"  Part A — auto-spawn: {'YES (Agent tool used)' if agent_used_a else 'NO (handled all steps itself)'}")
    print(f"  Part B — custom --agents: {'YES (Agent tool used)' if agent_used_b else 'NO (may not have dispatched)'}")
