...
def run_claude(prompt: str, *, max_turns: int = 3, extra_args: list[str] | None = None) -> list[dict]:
    ...
    cmd = ["claude", "-p", prompt, "--output-format", "stream-json", "--max-turns", str(max_turns)]
    if MODEL:
        cmd += ["--model", MODEL]
    if extra_args:
        cmd += extra_args
...
def main() -> None:
    ...
    print("KEY FINDINGS")
    print("=" * 60)
    print("  • CLI has ALL tools: Bash, Read, Write, Edit, Glob, Grep, TodoWrite,")
    print("    Agent, WebFetch, WebSearch, NotebookEdit, AskUserQuestion, etc.")
    print("  • The CLI is the full Claude Code experience — SDK is a subset of this")
